...
(async () => {
  ...
  async function parseClippings() {
    ...
    for (let index = 0; index < clippingBlocks.length; index++) {
      ...
      const latestTimestampINFile = await getLastInteractionFromFile(fileName);
      // we only add markdown that has been added after we last touched the file
      if (timestampStringGreaterThanOrEqual(latestTimestampINFile, timestamp))
        continue;
    ...
    await generateMarkdownFiles(bookData);
  ...
  function parseBlockData(block, index) {
    ...
    if (metadataLine.includes(NOTE_KEYWORD)) {
      ...
      console.warn(
        `WARN: Skipping block #${index + 1}. Unknown clipping type.`,
      );
      return null;
    }
...
\n`;
      ...
      upsertMarkdownFile(filePath, frontMatter, markdownBody);
      console.log(`- Successfully wrote: ${fileName}`);
    });
  }

  function upsertMarkdownFile(filePath, frontMatter, markdownBody) {
    if (fs.existsSync(filePath)) {
      // only append markdown to the end
      fs.readFile(filePath, "utf8", function (err, data) {
        if (err) {
          return console.log(err);
        }
        const newTimestamp = frontMatter.match(getLastInteractionRegex())[1];
        const oldTimestamp = data.match(getLastInteractionRegex())[1];
        const result = data.replace(oldTimestamp, newTimestamp);

        fs.writeFile(filePath, result, "utf8", function (err) {
          if (err) return console.log(err);
        });
      });
      fs.appendFileSync(filePath, markdownBody);
  ...
  function timestampStringGreaterThanOrEqual(stringA, stringB) {
  ...
  async function getLastInteractionFromFile(fileName) {
    try {
      ...
      const match = file.match(getLastInteractionRegex());
      if (match && match[1]) {
        return match[1];
      }
    } catch (e) {
  ...
  function getLastInteractionRegex() {
    return new RegExp(/last_interaction: "(.*)"/i);
  }
